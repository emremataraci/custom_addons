...
class BlanketOrder(models.Model):
    ...
    invoiced_total = fields.Monetary(
        compute='_compute_totals', store=True, string='Invoiced Total'
    )
    ordered_total = fields.Monetary(
        compute='_compute_totals', store=True, string='Ordered Total'
    )
    remaining_invoice_total = fields.Monetary(
        compute='_compute_totals', store=True, string='Remaining to Invoice Total'
    )
    remaining_total = fields.Monetary(
        compute='_compute_totals', store=True, string='Remaining Total'
    )
...
class BlanketOrderLine(models.Model):
    ...
    invoiced_subtotal = fields.Monetary(
        compute='_compute_subtotals', string='Invoiced Subtotal'
    )
    ordered_subtotal = fields.Monetary(
        compute='_compute_subtotals', string='Ordered Subtotal'
    )
    remaining_invoice_subtotal = fields.Monetary(
        compute='_compute_subtotals', string='Remaining Invoice Subtotal'
    )
    remaining_subtotal = fields.Monetary(
        compute='_compute_subtotals', string='Remaining Subtotal'
    )
    state = fields.Selection([
        ('draft', 'Draft'),
        ('open', 'Open'),
    ...
    client_order_ref = fields.Char(
        related='order_id.client_order_ref', store=True 
    )
    ...
    def _compute_display_name(self):
        if self.env.context.get("from_sale_order"):
            for record in self:
                name = "[%s]" % record.order_id.client_order_ref
