...
class PackagingPreparationReportXlsx(models.AbstractModel):
    ...
    def add_headers(self,sheet, batches, merge_format14, merge_format14left, merge_format10, merge_format18,row_line,header_format):
        ...
        for batch in batches:
            ...
            shipment_info_text = f'Shipment Number: {transportation_code} / Shipment Date:'
...
class PackagingDeliveryReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, batches):
        ...
        hs_codes = set()
        for batch in batches:
            # F24 hücresine 'DELIVERY DATE(ETA)' yazma
            sheet.write('F24', 'DELIVERY DATE(ETA)', bold_format)
            sheet.write('G24', batch.arrival_date or '', date_format)
            sheet.write('F25', 'TARIFF NUMBER', bold_format)

            for line in batch.packaging_preparation_ids:
                product = line.product_id
                hs_code = product.hs_code or product.categ_id.hs_code or ''
                if hs_code:
                    hs_codes.add(hs_code)

            hs_codes_str = ', '.join(sorted(hs_codes))
            sheet.write('G25', hs_codes_str)
            sheet.write('F26', 'SHIPMENT NUMBER', bold_format)
            sheet.write('G26', batch.transportation_code or '', date_format)
            hs_codes.clear()
...
class PackagingCustomsReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, batches):
        ...
        if response.status_code == 200:
            image_data = BytesIO(response.content)
            sheet.insert_image('A1:K6', 'YENA_logo.png', {'image_data': image_data, 'x_scale': 0.18, 'y_scale': 0.18})
        row += 1
        ...
        for customer in batch.customer_ids:
            ...
            # Teslimat adresini alma
            delivery_address_id = customer.address_get(['delivery']).get('delivery')
            if delivery_address_id:
                address_partner = self.env['res.partner'].browse(delivery_address_id)
            else:
                # Teslimat adresi yoksa müşterinin kendi adresini kullan
                address_partner = customer
            # Adres bilgisini oluşturma
            address = f"{address_partner.street or ''}, {address_partner.city or ''}, " \
              f"{address_partner.state_id.name or ''}, {address_partner.zip or ''}, " \
              f"{address_partner.country_id.name or ''}"
...
class PackagingBillOfLadingReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, batches):
        ...
        if response.status_code == 200:
            image_data = BytesIO(response.content)
            sheet.insert_image('A1:K5', 'YENA_logo.png', {'image_data': image_data, 'x_scale': 0.18, 'y_scale': 0.18})
        row += 1
        ...
        for batch in batches:
            ...
            for index, customer in enumerate(batch.customer_ids, start=1):
                customer_name = customer.name
                # Teslimat adresini alma
                address_id = customer.address_get(['delivery']).get('delivery')
                if address_id:
                ...
                # Adres bilgisini oluşturma
                delivery_address = f"{address.street or ''}, {address.city or ''}, " \
                                f"{address.state_id.name or ''}, " \
                                f"{address.zip or ''}, {address.country_id.name or ''}"
            ...
            sheet.merge_range('E7:F7', date_now, header_format_right)
            ...
            for header, detail in zip(address_headers, address_lines):
                ...
                sheet.merge_range(row, 6, row+1, 8, "adres'i kontrol et", note_format)
            ...
            sheet.merge_range(row, 2, row+2, 5, f"TOPLAM {len(unique_pallet_nos)} KAP", merge_format10)
            row+=3      
