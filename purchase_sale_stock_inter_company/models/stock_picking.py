...
class StockPicking(models.Model):
    ...
    intercompany_picking_id = fields.Many2one(
        comodel_name="stock.picking", string="Intercompany Picking"
    )

    def _action_done(self):
        po_picks = self.env['stock.picking']
        for pick in self.filtered(lambda x: x.location_dest_id.usage == "customer").sudo():
            ...
            for move_line in pick.move_line_ids:
                ...
                po_move_lines = sale_line.auto_purchase_line_id.move_ids.move_line_ids
                ...
                if qty_done and po_move_lines:
                    ...
                    raise UserError(
                        _(
                            ...
                            "quantity from %(pick_name)s for product %(product)s."
                        )
                        % {
                            "po": purchase.name,
                            "pick_name": pick.name,
